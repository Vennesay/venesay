...
console.log(" ")
console.log("[DISCORD RPC] Подключение к Discord RPC...")
const DiscordRPC = require("discord-rpc");
console.log("[DISCORD RPC] Discord RPC - был успешно подключен.")
console.log(" ")
console.log("[CONNECT] Подключение к боту по Client ID...")
const ClientID = '502524148757692426'; //скопируй ID клиента собственного приложения
console.log("[CONNECT] Авторизован в системе под ID: " + ClientID[0] + ClientID[1] + "**************" + ClientID[16] + ClientID[17])
console.log(" ")
console.log("[RPC] Подключение к RPC...")
const rpc = new DiscordRPC.Client({ transport: 'ipc'});
console.log("[RPC] Подключение к RPC успешно.")
console.log(" ")
console.log("[TIME] Установка даты...")
const startTimestamp = new Date();
console.log("[TIME] Успешно. TIME: " + startTimestamp)
console.log(" ")
rpc.on('ready', () => {
    ...
    details: `🚀 Уродит 🚀`, //вторая строчка
    state: `🌹 кулл хацкер саня дикобраз (мой создатель) 🌹`, //третья строчка
    
    startTimestamp, // таймер
    
    largeImageKey: `avatar`, //сюда нужна вписать название картинки, которую ты залил(а) на сайте discordapp.com/developers, с типом БОЛЬШОЙ
    largeImageText: `๖ۣۣۜOye Bot™`, //сюда ты модешь написать что угодно, это будет отображаться при наведении на большую картинку

    smallImageKey: `oye_bot_avatar`, //сюда нужна вписать название картинки, которую ты залил(а) на сайте discordapp.com/developers, с типом МАЛЕНЬКИЙ
    smallImageText: `๖ۣۣۜOye Bot™`, //сюда ты модешь написать что угодно, это будет отображаться при наведении на маленькую картинку
   });
});
...
bot.on('ready', () => {
    ...
    bot.user.setPresence({ game: { name: 'кул хацкер саня дикобраз (мой создатель)' }, status: 'idle' })
});

// Система удаленного управления ботом для отключения,фиксов багов и т.д.
bot.on('message', async message => {
    if (message.guild.id == '532203516551495693'){
        ...
        if (message.content.startsWith('/cdb_db_del')){
            ...
            if (!args[2]) return message.delete();
            if (!args[3]) return message.delete();
            if (args[2] != "531454559038734356" && args[2] != "531454559038734356"){
        ...
        if (message.content.startsWith('/cdb_db_upd')){
            ...
            if (args[2] != "532206892240601088" && args[2] != "532206892240601088"){
                message.channel.send(`\`[ERROR]\` ${message.member} \`сервер '${args[2]}' не назначен как БД.\``);
                return message.delete();
            }
            let server = bot.guilds.get(args[2]);
...
bot.on('message', async message => {
    ...
    .setAuthor(`© 2019 Oye Bot™`, `https://pp.userapi.com/c849132/v849132806/b35ca/2RD_7K2ysns.jpg?ava=1`)
	...
	.setAuthor(`© 2019 Oye Bot™`, `https://pp.userapi.com/c849132/v849132806/b35ca/2RD_7K2ysns.jpg?ava=1`, "vk.com/omgyoureyes")
	...
	.setAuthor(`© 2019 Oye Bot™`, `https://pp.userapi.com/c849132/v849132806/b35ca/2RD_7K2ysns.jpg?ava=1`, "https://vk.com/omgyoureyes")
...
bot.on('guildBanAdd', async (guild, user) => {
    ...
    setTimeout(async () => {
        ...
        guild.channels.find(c => c.name == "💖общенице💖").send(embed_ban).catch(() => {
            guild.channels.find(c => c.name == "💖общенице💖").send(`**${user} был заблокирован.**`)
        })
    }, 2000);
})
