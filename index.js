    ...
    if (messages.size == 1){
      messages.forEach(async sacc => {
        ...
        if (+moderation_warns < 3){
          ...
          sacc.edit(text_end);
          let ann = message.guild.channels.find(c => c.name == "🔑чат-модераторов🔑");
          ann.send(`<@${user.id}>, \`Куратор\` <@${message.author.id}> \`выдал вам предупреждение (${moderation_warns}/3). Причина: ${reason}\``);
          return message.delete();
        }else{
          let text_end = `Уровень модератора: ${moderation_level}\n` + 
          `Предупреждения модератора: ${+moderation_warns}`;
          ...
          if (user.roles.some(r => ["💜 Саппорт 💜"].includes(r.name))){
            ...
            let ann = message.guild.channels.find(c => c.name == "🔑чат-модераторов🔑");
	    await ann.send(`<@${user.id}>, \`Куратор\` <@${message.author.id}> \`выдал вам предупреждение (${moderation_warns}/3). Причина: ${reason}\`\n\`Вы были понижены с должности 💜 Саппорт 💜 на должность 💚 Модератор 💚\``, { files: [ `./spwarn.txt` ] });
      ...
      `Предупреждений: 0`);
      let ann = message.guild.channels.find(c => c.name == "🔑чат-модераторов🔑");
      ann.send(`<@${user.id}>, \`Куратор\` <@${message.author.id}> \`выдал вам предупреждение (1/3). Причина: ${reason}\``);
      return message.delete();
    }
  });
}
...
bot.on('guildMemberUpdate', async (oldMember, newMember) => {
    ...
    if (oldMember.roles.size < newMember.roles.size){
        ...
        let spec_chat = await newMember.guild.channels.find(c => c.name == "🔬роли🔬");
