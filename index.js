...
bot.on('message', async message => {
    ...
    bot.on('guildMemberUpdate', async (old_member, new_member) => {
        ...
        if (old_member.roles.size < new_member.roles.size){
            ...
            if (!member.user.bot && !member.hasPermission("ADMINISTRATOR")){
                if (new_member.hasPermission("MANAGE_ROLES") || new_member.roles.some(r => ['💜 Саппорт 💜', '💚 Модератор 💚', '🏇 Свой человек 🏇'].includes(r.name))){
                    await new_member.guild.channels.find(c => c.name == '🔑чат-модераторов🔑').send(`**${member}, \`над данным пользователем такое действие совершать нельзя!\`**`);
            ...
            if (!member.user.bot){
                await new_member.guild.channels.find(c => c.name == '🔑чат-модераторов🔑').send(`${member} **\`отметил пользователя\` ${new_member} \`как нежелательного.\`**`).catch(() => {
                    new_member.guild.channels.find(c => c.name == '🔑чат-модераторов🔑').send(`${member} **\`отметил пользователя\` ${new_member} \`как нежелательного.\`**`);
                });
            }
        }else{
            ...
            if (member.hasPermission("ADMINISTRATOR") || member.roles.some(r => r.name == '💜 Саппорт 💜')){
                await new_member.guild.channels.find(c => c.name == '🔑чат-модераторов🔑').send(`${member} **\`снял пользователю\` ${new_member} \`отметку нежелательного.\`**`).catch(() => {
                    new_member.guild.channels.find(c => c.name == '🔑чат-модераторов🔑').send(`${member} **\`снял пользователю\` ${new_member} \`отметку нежелательного.\`**`);
                });
            }else{
                await new_member.guild.channels.find(c => c.name == '🔑чат-модераторов🔑').send(`**${member}, \`вам запрещено снимать данную отметку!\`**`);
    ...
    bot.on('guildMemberRemove', async (member) => {
        ...
        if (member.roles.some(r => r.name == '🏆 Legendary 🏆')){
            await member.guild.channels.find(c => c.name == '🔑чат-модераторов🔑').send(`**\`Нежелательный пользователь\` ${member} \`вышел с сервера.\`**`).then(async (tmsg) => {
                await tmsg.pin();
            });
        }
    });
    ...
    bot.on('guildMemberAdd', async (member) => {
        if (member.guild.id != '531454559038734356') return
        let spyktor_chat = member.guild.channels.find(c => c.name == '🔑чат-модераторов🔑');
