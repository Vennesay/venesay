...
bot.on('message', async message => {
    ...
    if (message.content.startsWith("/report")){
        ...
        message.delete();
        return message.guild.channels.find(c => c.name == "🔞spectators-chat🔞").send(`\`Появился новый вопрос №${rep_number}. Используйте '/ans' что бы ответить. '/questions' - список активных вопросов.\``).then(msg => msg.delete(120000))
    }
    ...
    */
    ...
    if (message.content.toLowerCase().startsWith("/remove")){
        if (!message.member.roles.some(r=>["🌈 Администратор 🌈", "✵Leader✵", "✮Ministers✮", "Spectator™", "✔ Helper ✔", "Support Team", "✔Jr.Administrator✔", "✔ Administrator ✔"].includes(r.name)) && !message.member.hasPermission("ADMINISTRATOR")) return
        let user = message.guild.member(message.mentions.users.first());
        if (!user){
            message.delete();
        ...
        let countroles = 0;
        let rolesgg = ["🎮 Геймер 🎮", "⋆ Department of Justice ⋆", "⋆ Department of Defence ⋆", "⋆ Department of Health ⋆", "⋆ Mass Media ⋆", "⋆ Warlock MC ⋆", "⋆ Russian Mafia ⋆", "⋆ La Cosa Nostra ⋆", "⋆ Yakuza ⋆", "⋆ Grove Street Gang ⋆", "⋆ East Side Ballas Gang ⋆", "⋆ Vagos Gang ⋆", "⋆ Aztecas Gang ⋆", "⋆ Rifa Gang ⋆", "⋆ Night Wolfs ⋆"]
        for (i in rolesgg){
        ...
        if (countroles > 1){
            ...
            bot.guilds.find(g => g.id == message.guild.id).channels.find(c => c.name == "🔞spectators-chat🔞").send(`<@${user.id}> \`у вас забрали фракционные роли, так как их количество привышало допустимое значение.\``)
        }else{
