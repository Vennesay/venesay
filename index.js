...
bot.on('message', async message => {
    ...
    if (message.content == '/reset_ddos'){
        if (!message.member.roles.some(r => r.name == "💚 Модератор 💚") && !message.member.roles.some(r => r.name == "💜 Саппорт 💜") && !message.member.roles.some(r => r.name == "💙 Куратор 💙") && !message.member.roles.some(r => r.name == "💘 Одмен 💘")) return message.reply("нет прав")
        levelhigh = 0;
