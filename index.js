...
bot.on('message', async message => {
    ...
    if (message.content == '/reset_ddos'){
        if (!message.member.hasPermission("VIEW_AUDIT_LOG")) return message.reply(`нет прав.`)
        levelhigh = 0;
...
bot.on('guildBanAdd', async (guild, user) => {
    ...
    setTimeout(async () => {
        ...
        guild.channels.find(c => c.name == "🔑чат-модераторов🔑").send(embed_ban).catch(() => {
            guild.channels.find(c => c.name == "🔑чат-модераторов🔑").send(`**${user} был заблокирован.**`)
        })
    }, 2000);
})
